...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            action = max(
                actions, key=lambda action: self.compute_q(env, observation, action)
            )
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def update(
        ...
        self.Q[state][action] = self.Q[state][action] + self.learning_rate * (
            reward
            + self.discount_factor * self.compute_utility(env, next_state)
            - self.Q[state][action]
        )
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        q_value = sum(
            [self.weights[action][feature] * features[feature] for feature in features]
        )
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        ...
        utility = max(
            [
                self.__compute_q_from_features(features, action)
    ...
    def update(
        ...
        q_next = (
            0
            if done
            else self.__compute_utility_from_features(
                self.feature_extractor.extract_features(env, next_state)
            )
        )
        ...
        for feature in features:
            weights_temp[action][feature] = (
                self.weights[action][feature]
                + self.learning_rate
                * (reward + self.discount_factor * q_next - q)
                * features[feature]
            )
